<?php
// Top-level precomputed variables to keep template logic clear
$currentStep = $fluxos[$currentId] ?? [];
$questionNormalized = function_exists('mb_strtolower')
    ? mb_strtolower(trim($currentStep['pergunta'] ?? ''), 'UTF-8')
    : strtolower(trim($currentStep['pergunta'] ?? ''));
...
<x-filament-panels::page>
    ...
    <style>
        ...
        .btn-ghost:hover { background-color: #f9fafb; color: #1f2937; transform: translateY(-2px); }
        ...
        .finalize-btn .finalize-btn-icon { width: 2rem; height: 2rem; }

        .finalize-btn .finalize-text { display:block; text-align:left; white-space:normal; height: 40px; }
        .finalize-btn .finalize-text .small,
        .finalize-btn .finalize-text .muted { display:block; font-size:1rem; line-height:1.05; opacity:0.92; }
        .finalize-btn .finalize-text .small { font-weight:500; margin-top: 1.25px; margin-bottom: 2.5px; }
        ...
        .option-group > button.btn-voltar:hover { background-color: #f8fafc !important; color: #1f2937 !important; transform: translateY(-2px); }

        /* ---------- Cards & layout ---------- */
        .form-card { width:100%; max-width:920px; position:relative; border-radius:1.25rem; overflow:visible; margin:0 auto; box-shadow:0 18px 40px rgba(2,6,23,0.45); }
        .form-card-inner {
            background-color:#0C0A39; color:#fff; border-radius:1.25rem; overflow:hidden;
            border:1px solid rgba(255,255,255,0.06); padding:1.5rem 11rem 2rem 1.5rem; position:relative; box-sizing:border-box;
        }
        ...
        .completion-card {
            width: min(1100px,94%); background-color:#f3f4f6; padding:2.5rem 3rem; border-radius:1.75rem;
            box-shadow:0 28px 60px rgba(2,6,23,0.28); border:1px solid rgba(245,158,11,0.12); text-align:center; margin:auto 0;
        }
        ...
        .fi-layout-panel { height:100% !important; display:flex !important; flex-direction:column !important; justify-content:center !important; align-items:center !important; overflow-y:auto !important; padding:0 !important; margin:0 !important; }

        .page-container { width:100%; min-height:calc(100vh - var(--bottom-panel-height)); display:flex; flex-direction:column; align-items:center; justify-content:center; padding:1rem; box-sizing:border-box; position:relative; gap:1.25rem; }
        .form-card, .completion-card { margin-top:0; margin-bottom:0; }

        /* Bottom panel */
        .bottom-panel { position:fixed; left:55%; transform:translateX(-50%); bottom:1.5rem; z-index:60; width:calc(100% - 4rem); max-width:1400px; pointer-events:none; }
        .bottom-panel .inner { display:flex; gap:1rem; align-items:flex-end; justify-content:space-between; pointer-events:auto; }
        .bottom-panel .card { background:rgba(255,255,255,0.96); border-radius:1rem; padding:0.85rem; box-shadow:0 18px 40px rgba(2,6,23,0.08); border:1px solid rgba(15,23,42,0.06); display:flex; flex-direction:column; gap:0.5rem; }
        ...
        .info-row { display:grid; grid-template-columns:repeat(3, 1fr); gap:0.6rem; }
        .info-field { background:linear-gradient(180deg,#fff,#fbfbfd); border-radius:0.75rem; padding:0.65rem 0.75rem; display:flex; gap:0.6rem; align-items:center; border:1px solid rgba(15,23,42,0.04); }
        ...
        .info-hover-panel {
            position:absolute; bottom:100%; left:0; width:100%; margin-bottom:12px; background-color:#fff; border-radius:1rem;
            padding:0.85rem; box-shadow:0 20px 45px -10px rgba(2,6,23,0.2); border:1px solid rgba(15,23,42,0.08);
            opacity:0; visibility:hidden; pointer-events:none; transition:all 0.2s ease-out; z-index:70; transform:translateY(10px); display:flex; flex-direction:column; gap:0.6rem;
        }
        .info-hover-panel::after { content:''; position:absolute; bottom:-6px; left:50%; transform:translateX(-50%) rotate(45deg); width:12px; height:12px; background-color:#fff; border-right:1px solid rgba(15,23,42,0.08); border-bottom:1px solid rgba(15,23,42,0.08); }
        .info-wrapper:hover .info-hover-panel { opacity:1; visibility:visible; pointer-events:auto; transform:translateY(0); }

        /* BOTTOM-PANEL +2px FONT PATCH (SCOPED) */
        .bottom-panel .info-field .label { font-size: calc(0.78rem + 1px); }
        .bottom-panel .info-field .value { font-size: calc(0.95rem + 1px); }
        .bottom-panel .info-field .info-icon { width:19px !important; height:20px !important; }
        .bottom-panel .summary .summary-title h3 { font-size: calc(1.05rem + 1px); }
        .bottom-panel .summary .summary-count { font-size: calc(0.85rem + 1px); }
        .bottom-panel .summary .summary-item .q { font-size: calc(0.95rem + 1px); }
        .bottom-panel .summary .summary-item .a { font-size: calc(0.9rem + 1px); }
        .bottom-panel .competence .label { font-size: calc(0.85rem + 1px); }
        .bottom-panel .competence select { font-size: calc(0.95rem + 1px); }
        /* end patch */
    </style>
            ...
            <div class="completion-card" x-transition.opacity.scale.duration.400ms>
                <div class="pt-6 flex flex-col items-center justify-center">
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row gap-4 pb-6 justify-center">
                        <a href="{{ route('filament.admin.resources.processos.buscae-apreensao.tarefa-minutar-sentencas.create', ['statusprocesso_id' => $statusProcessoAtual ?? 2]) }}"
                        ...
                        <div class="flex gap-6">
                            <div class="flex-1 relative">
                                ...
                                <div class="mt-6 flex items-center flex-wrap gap-3">
                                    {{-- se for passo com opções sim/nao --}}
                                    @if (isset($currentStep['sim']) || isset($currentStep['nao']))
                                    ...
                                    @if (!isset($currentStep['sim']) && !isset($currentStep['nao']) && isset($currentStep['modelo']))
                                        ...
                                        <button type="button"
                                                wire:click="finalizeAndOpenModelo({{ $currentId }})"
                                                class="finalize-btn btn-secondary">
                                    ...
                                    {{-- botão voltar sempre que houver histórico --}}
                                    @if(count($history) > 0)
                ...
                <div class="info-wrapper card info" role="region" aria-label="Informações do processo">
                    <div class="info-hover-panel">
                        <div class="info-row">
                            <div class="info-field">
                                ...
                                <div><div class="label">Dias Estacionado</div><div class="value">...</div></div>
                            </div>
                            ...
                            <div class="info-field">
                                ...
                                <div><div class="label">Data Inicial</div><div class="value">...</div></div>
                            </div>
                            ...
                            <div class="info-field">
                                ...
                                <div><div class="label">Data de Conclusão</div><div class="value">...</div></div>
                            </div>
                        </div>
                        ...
                        <div class="info-row">
                            <div class="info-field">
                                ...
                                <div><div class="label">Advogados</div><div class="value">...</div></div>
                            </div>
                            ...
                            <div class="info-field">
                                ...
                                <div><div class="label">Nome da Parte</div><div class="value">...</div></div>
                            </div>
                    ...
                    <div class="info-row">
                        <div class="info-field" title="Data">
                            <svg class="info-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line></svg>
                            <div><div class="label">Data</div><div class="value">{{ now()->format('d/m/Y') }}</div></div>
                        </div>

                        <div class="info-field" title="Hora">
                            <svg class="info-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                            <div><div class="label">Hora</div><div class="value" x-text="currentTime">{{ now()->format('H:i') }}</div></div>
                        </div>
                        ...
                        <div class="info-field" title="Tarefa">
                            ...
                            <div><div class="label">Classe: Busca e Apreensão</div><div class="value">{{ $this->getHeading() }}</div></div>
                        </div>
                    </div>
                    ...
                    <div class="info-row">
                        <div class="info-field" title="Nº do Processo">
                            ...
                            <div><div class="label">Nº do Processo</div><div class="value">00000000000000000000</div></div>
                        </div>
                        ...
                        <div class="info-field" title="Usuário">
                            <svg class="info-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                            <div><div class="label">Usuário</div><div class="value">{{ auth()->user()->name ?? 'Usuário' }}</div></div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('livewire:load', function () {
            ...
            // limpa seleção no Alpine quando necessário
            Livewire.on('reset-selection', function () {
                // se estiver usando Alpine entangled prop 'selectedOption', setar para null no Alpine
                try {
                    // se store não existir, apenas setar a prop local
                    if (window.Alpine && Alpine.store && Alpine.store('selectedOption') !== undefined) {
                        Alpine.store('selectedOption', null);
                    } else {
                        // fallback: dispatch custom event para atualizar o componente
                        document.dispatchEvent(new CustomEvent('reset-selection'));
                    }
                } catch (e) {
                    // noop
                }
