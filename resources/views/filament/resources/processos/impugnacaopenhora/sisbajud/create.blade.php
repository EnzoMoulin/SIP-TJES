<x-filament-panels::page>
    ...
    <style>
        ...
        .summary-wrapper { position: absolute; bottom: 2rem; left: calc(var(--fi-sidebar-width, 20rem) + 2rem); z-index: 50; width: min(380px, 88%); max-width: 420px; transition: transform .22s ease, opacity .22s ease; backdrop-filter: blur(6px); }
        .summary-panel { background: rgba(255,255,255,0.92); border-radius: 0.9rem; box-shadow: 0 18px 40px rgba(2,6,23,0.12); border: 1px solid rgba(15,23,42,0.06); padding: 1rem; overflow: hidden; }
        .summary-header { display: flex; align-items: center; justify-content: space-between; gap: 0.5rem; margin-bottom: 0.65rem; }
        .summary-title { display: flex; gap: 0.5rem; align-items: baseline; }
        .summary-title h3 { margin: 0; font-size: 1.05rem; font-weight: 700; color: #0f172a; }
        .summary-count { font-size: .85rem; color: #475569; background: rgba(15,23,42,0.04); padding: .2rem .5rem; border-radius: 999px; font-weight: 600; }
        .summary-controls { display: flex; gap: .45rem; align-items: center; }
        .summary-toggle { background: transparent; border: 0; padding: .35rem; border-radius: .5rem; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: background .12s ease, transform .12s ease; }
        .summary-toggle:hover { background: rgba(2,6,23,0.04); transform: translateY(-2px); }
        .summary-list { max-height: 42vh; overflow-y: auto; padding-right: .25rem; }
        .summary-list ol { margin: 0; padding: 0; list-style: none; display: flex; flex-direction: column; gap: .6rem; }
        .summary-item { display: flex; gap: .6rem; align-items: flex-start; padding: .55rem; border-radius: .6rem; transition: background .12s ease, transform .08s ease; }
        .summary-item:hover { background: rgba(2,6,23,0.02); transform: translateY(-2px); }
        .summary-item .q { flex: 1 1 auto; color: #0f172a; font-weight: 600; font-size: .95rem; line-height: 1.15; }
        .summary-item .a { flex: 0 0 auto; align-self: center; font-size: .82rem; font-weight: 700; padding: .28rem .5rem; border-radius: 999px; text-transform: uppercase; width: 3.5rem; min-width: 3.5rem; text-align: center; }
        ...
        .page-container { width: 100%; flex-grow: 1; display: flex; justify-content: center; align-items: center; padding: 1rem; box-sizing: border-box; position: unset; top: unset; left: unset; transform: unset; }
        .form-card, .completion-card { margin-top: auto; margin-bottom: auto; }
        ...
        @media (max-width: 920px) { .summary-wrapper { left: 1rem; right: 1rem; width: auto; max-width: calc(100% - 2rem); bottom: 1rem; } .summary-panel { padding: .75rem; } .summary-list { max-height: 34vh; } }
        @media (max-width: 480px) { .summary-wrapper { bottom: .75rem; left: .5rem; right: .5rem; } .summary-panel { padding: .6rem; border-radius: .75rem; } .summary-title h3 { font-size: 1rem; } .summary-item .q { font-size: .9rem; } }
                        ...
                        <ol>
                            <?php
                                foreach ($history as $questionId) {
                                    if (!isset($fluxos[$questionId]) || !isset($responses[$questionId])) continue;
                                    ...
                                    $pergunta = $fluxos[$questionId]['pergunta'] ?? '';
                            ?>
