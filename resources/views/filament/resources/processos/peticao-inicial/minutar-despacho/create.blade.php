...
.icon-voltar { color: #dc2626; } /* seta vermelha conforme solicitado */
...
/* ---------- Cards & layout (mantidos em grande parte) ---------- */
.form-card {
    width: 100%;
    max-width: 920px;
    position: relative;
...
/* Summary styling (volta ao layout anterior) */
...
<div x-data="{ selectedOption: @entangle('selectedOption') }"
    ...
    {{-- BOTTOM PANEL: reúne Resumo | Info | Selecionar Competência (fixo e centralizado)
         NÃO RENDERIZAR quando $isComplete for true --}}
    @unless ($isComplete)
