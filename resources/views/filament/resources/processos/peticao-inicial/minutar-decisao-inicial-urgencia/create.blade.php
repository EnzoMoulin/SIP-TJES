<x-filament-panels::page>
    ...
    <style>
        ...
        .option-group > button.option-group-btn-inactive:hover { background-color: #f8fafc !important; color: #1f2937 !important; transform: translateY(-2px); }
    ...
    <div x-data="{ selectedOption: @entangle('selectedOption') }"
        ...
        @else
            @if ($currentId && isset($fluxos[$currentId]))
                ...
                {{-- Novo código para verificar o rótulo do botão --}}
                @php($customLabel = $this->getCustomActionLabel($currentStep['id']))
                
                {{-- Verifica se o passo atual é um dos IDs customizados (10 ou 11) --}}
                @php($showModeloButton = in_array($currentStep['id'], $this::FLUXO_IDS_CONTINUAR ?? []))
                        ...
                        <div class="flex gap-6">
                            <div class="flex-1 relative">
                                ...
                                <div class="mt-6 flex items-center flex-wrap gap-3">
                                    ...
                                    {{-- INÍCIO: Se o rótulo é customizado (Continuar) --}}
                                    @if ($customLabel)
                                        
                                        {{-- 1. Botão 'Abrir Modelo' (À ESQUERDA - GRANDE) --}}
                                        @if ($showModeloButton)
                                             <button type="button" wire:click="openModeloOnly({{ $currentId }})" class="finalize-btn">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="finalize-btn-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                </svg>
                                        ...
                                        {{-- 2. Botão 'Continuar' (GRANDE, usaremos base-btn) --}}
                                        <button type="button"
                                                @click="selectedOption = 'sim'; $wire.selectOption('sim')"
                                                class="base-btn"
                                                :class="{
                                                    'btn-secondary': selectedOption === 'sim', 
                                                    'btn-ghost': selectedOption !== 'sim' 
                                                }">
                                    ...
                                    @elseif (isset($currentStep['sim']) || isset($currentStep['nao']))
                                        {{-- Opções 'Sim' / 'Não' Padrão (Manter Option-Group) --}}
                                        <div role="group" class="option-group" aria-label="Opções: Sim / Não">
                                            @if (isset($currentStep['sim']))
                                                <button type="button"
                                                        @click="selectedOption = 'sim'; $wire.selectOption('sim')"
                                                        :class="{
                                                            'option-group-btn option-group-btn-active': selectedOption === 'sim', 
                                                            'option-group-btn option-group-btn-inactive': selectedOption !== 'sim' 
                                                        }">
                                                     <svg class="icon-sim" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                                            ...
                                            @if (isset($currentStep['nao']))
                                                <button type="button"
                                                        ...
                                                        :class="{
                                                            'option-group-btn option-group-btn-active': selectedOption === 'nao', 
                                                            'option-group-btn option-group-btn-inactive': selectedOption !== 'nao' 
                                                        }">
                                    ...
                                    {{-- Botão Abrir Modelo Padrão (para passos finais NÃO customizados) --}}
                                    @if (!isset($currentStep['sim']) && !isset($currentStep['nao']) && isset($currentStep['modelo']) && !$showModeloButton)
                                    ...
                                    @elseif (!isset($currentStep['sim']) && !isset($currentStep['nao']) && !isset($currentStep['modelo']))
                                        {{-- Botão Finalizar Padrão (caso não seja customizado nem tenha modelo) --}}
                                        <button type="button" wire:click="finalizeOnly()" class="base-btn btn-secondary">Finalizar e Concluir</button>
                                    @endif
                                    
                                    @if(count($history) > 0)
                                        {{-- Botão Voltar (Com seta vermelha) --}}
                                        <div role="group" class="option-group">
                                            <button type="button" wire:click="goBack" class="option-group-btn option-group-btn-inactive voltar-btn">
                            ...
                            <?php
                                if (!empty($history) && is_array($history)) {
                                    foreach ($history as $questionId) {
                                        ...
                                        $customLabelForSummary = in_array($questionId, [10, 11]) ? 'CONTINUAR' : null;

                                        if ($customLabelForSummary) {
                                            $normalizedResponse = 'CONTINUAR / MODELO';
                                            $pillClass = 'pill-otro';
                                        } elseif (in_array($answerLower, ['nao', 'não'], true)) {
